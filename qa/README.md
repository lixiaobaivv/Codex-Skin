...
JSON 会报告主题 ID、首页/侧栏/样式是否存在、视口、横向溢出、hero 背景长度以及控制台错误，便于批量验收四套 v2/v3 生图 hero。
...
测试结束后直接关闭这份独立浏览器配置即可移除运行时页面。`apply` 已更新本机保存主题；如需同时清理保存状态和 hook，再执行 `.\Codex主题商店.exe rollback`。
