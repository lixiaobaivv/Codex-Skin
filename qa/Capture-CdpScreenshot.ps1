...
try {
    ...
    $state = Invoke-CdpCommand "Runtime.evaluate" @{
        expression = "(() => { const home = document.getElementById('codex-theme-home'); const hero = document.querySelector('.codex-theme-home-hero'); const editor = document.querySelector('.ProseMirror'); const composer = document.querySelector('.composer-surface-chrome'); const logo = document.querySelector('.codex-theme-masthead-logo'); const pet = document.querySelector('.codex-theme-pet-image'); const heroImage = hero ? getComputedStyle(hero).backgroundImage : ''; const imageVariable = getComputedStyle(document.documentElement).getPropertyValue('--codex-theme-background-image'); const homeRect = home?.getBoundingClientRect(); const composerRect = composer?.getBoundingClientRect(); const visibleCards = [...document.querySelectorAll('.codex-theme-home-action')].filter(card => { const rect = card.getBoundingClientRect(); return !!homeRect && rect.bottom > homeRect.top && rect.top < homeRect.bottom; }); const cardComposerOverlap = !!homeRect && !!composerRect && visibleCards.some(card => { const rect = card.getBoundingClientRect(); const visibleBottom = Math.min(rect.bottom, homeRect.bottom); const visibleTop = Math.max(rect.top, homeRect.top); return visibleBottom > composerRect.top && visibleTop < composerRect.bottom && rect.right > composerRect.left && rect.left < composerRect.right; }); return { themeId: document.documentElement.dataset.codexThemeId || '', homeVisible: home ? getComputedStyle(home).display !== 'none' : false, legacySidebar: !!document.getElementById('codex-theme-sidebar'), style: !!document.getElementById('codex-theme-store-live-style'), viewport: [innerWidth, innerHeight], editorText: (editor?.textContent || '').trim(), quickActionCount: document.querySelectorAll('.codex-theme-home-action').length, horizontalOverflow: document.documentElement.scrollWidth > innerWidth || document.body.scrollWidth > innerWidth, homeScroll: home ? [home.clientHeight, home.scrollHeight] : [0, 0], homeBottom: homeRect ? Math.round(homeRect.bottom) : 0, composerTop: composerRect ? Math.round(composerRect.top) : 0, cardComposerOverlap, logoLoaded: !!logo && logo.complete && logo.naturalWidth > 0, petLoaded: !!pet && pet.complete && pet.naturalWidth > 0, logoNaturalSize: logo ? [logo.naturalWidth, logo.naturalHeight] : [0, 0], logoSourceLength: logo?.currentSrc?.length || 0, heroImageLength: heroImage.length, heroImagePrefix: heroImage.slice(0, 80), imageVariableLength: imageVariable.length, imageVariablePrefix: imageVariable.slice(0, 80) }; })()"
        returnByValue = $true
    }
