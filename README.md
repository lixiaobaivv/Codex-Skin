...
| 系统 | 推荐文件 | 说明 |
| --- | --- | --- |
| Windows x64 | `Codex-Skin-Setup-win-x64.exe` | 推荐，完整安装并自动注册网页导入协议 |
| Windows x64 便携版 | `Codex-Skin-win-x64.exe` | 单文件运行，需要手动注册网页导入协议 |
| macOS Apple Silicon | `Codex-Skin-osx-arm64.pkg` | 适用于 M1/M2/M3/M4 等 Apple 芯片 |
| macOS Intel | `Codex-Skin-osx-x64.pkg` | 适用于 Intel Mac |

ZIP 是 GitHub Actions 或便携分发的外层包装，不是推荐安装方式。普通用户优先下载 Setup EXE 或 PKG。
...
把文件名替换为 Release 页面显示的实际值。镜像属于第三方服务，可能失效或缓存旧文件；下载后请使用同一 Release 中的 `Codex-Skin-win-x64-SHA256SUMS.txt` 或 `Codex-Skin-installers-SHA256SUMS.txt` 校验文件。校验不一致时不要运行。
...
2. 请求用户确认下载；
3. 限制 HTTPS 重定向、下载大小和网络目标；
4. 校验整包 SHA-256、Ed25519 签名、主题清单和图片；
5. 原子安装主题；
6. 再次询问是否重启 Codex 并应用。

网页点击不会静默安装或切换主题。Windows Setup 和 macOS PKG 会注册协议；Windows 便携版需要先执行 `protocol register`。

验证完成的主题会保留在本机主题列表中，重新打开 Codex-Skin 后仍可选择；同一主题存在多个版本时自动使用最高版本。

也可以双击本地 `.dreamskin` 文件，或在 Windows 命令行导入：
...
客户端启动时自动刷新，也可以手动刷新。更新会先下载到临时目录，全部校验通过后才替换缓存；失败时继续使用上一次有效目录或内置主题。
...
切换 GitHub、GH Proxy 或 GHFast 后重试。同步失败不会删除内置主题和上一次有效缓存。

### 网页点击“一键导入”没有反应

- Windows Setup：重新运行安装器修复协议关联，或执行 `protocol register`。
- Windows 便携版：先执行 `protocol register`。
